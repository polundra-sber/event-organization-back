...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<UserAlreadyExistResponse> handleUserAlreadyExists(UserAlreadyExistsException ex) {
        UserAlreadyExistResponse response = new UserAlreadyExistResponse().error(ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
    }

    @ExceptionHandler(BadCredentialsException.class)
    public ResponseEntity<AuthUnsuccessResponse> handleUserAlreadyExists(BadCredentialsException ex) {
